...
class DihedralMaker(TrajectoryProcess):
    ...
    def __init__(self, host, strand_id, big_traj_folder, backbone_data_folder):
        ...
        self.plumed_test_folder = path.join('/home/yizaochen/codes/backbone_rigidity/plumed_test', host)
        self.check_folder_in_dihedralmaker()


    def check_folder_in_dihedralmaker(self):
        for folder in [self.plumed_in_folder, self.plumed_out_folder, self.plumed_test_folder]:
    ...
    def make_all_out_for_test(self, dihedral_name):
        resid_lst = self.d_resid_lst[self.strand_id]
        for resid in resid_lst:
            f_input = self.make_plumed_input_for_test(resid, dihedral_name)
            cmd = f'plumed driver --plumed {f_input} --mf_pdb {self.all_pdb}'
            print(cmd)
            system(cmd)
    ...
    def make_plumed_input_for_test(self, resid, dihedral_name):
        f_name = path.join(self.plumed_test_folder, f'{dihedral_name}.{resid}.dat')
        f_out = path.join(self.plumed_test_folder, f'{dihedral_name}.{resid}.out')
        indices_text = self.get_indices_by_resid_dihedral_name(resid, dihedral_name)
        f = open(f_name, 'w')
        f.write(f'phi1: TORSION ATOMS={indices_text}\n')
        f.write(f'PRINT ARG=phi1 FILE={f_out} STRIDE=1\n')
